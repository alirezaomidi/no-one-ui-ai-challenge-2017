...
def put_strategy(game):
    ...
    # Enemy has more than 1 dooz opportunity
    # Try to find a two-way
    for empty_pos in game.get_empty_positions():
        my_opps = game.dooz_opportunities(game.get_my_positions() + [empty_pos])
        if len(my_opps) > 1:
    ...
    if game.get_board().get_emptycells():
        ...
        return game.put(random.choice(game.get_board().get_emptycells()).get_pos())
